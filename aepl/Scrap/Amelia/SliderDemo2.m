function SliderDemo2(MyImHandle)
%clc
%clear all
%disp(class(MyImHandle))
NumFrames = size(MyImHandle.data,ndims(MyImHandle.data)); %4; %// Check below for dummy 4D matrix/image sequence
...
%MyImHandle.data = imread();

%MyImHandle.data = cat(3,MyImHandle.data(:,:,:));
%size(MyImHandle.data,3),MyImHandle.data(:,:,:));
...
setappdata(hFig,'MyIm',MyImHandle.data); %// You could use %//setappdata(0,'MyImHandle.data',MyImHandle.data) to store in the base workspace. 

%// Display 1st frame
imshow(MyImHandle.data(:,:,1),'InitialMagnification','fit','Border','tight')
...
guidata(hFig,handles);
    ...
    function XListenerCallBack
        ...
        %// Here retrieve MyImHandle.data using getappdata.
        %MyImHandle.data = getappdata(hFig,'MyImHandle.data');
        ...
        imshow(MyImHandle.data(:,:,CurrentFrame),'Parent',handles.axes1,'InitialMagnification','fit');
